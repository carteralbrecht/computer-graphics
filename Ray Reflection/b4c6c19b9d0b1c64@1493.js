// https://observablehq.com/@spattana/skybox-from-cubemap-and-hemisphere-lighting@1493
...
regl({
  ...
  main.variable(observer("renderObject")).define("renderObject", ["regl","object","viewMatrix","perspectiveMatrix","hex2rgb"], function(regl,object,viewMatrix,perspectiveMatrix,hex2rgb){return(
regl(
  {
  vert: `
    precision mediump float;
  ...
  frag: `
    ...
    void main () {
      ...
      vec3 up = vec3(0,1,0); // Hardcoded Up of the hemisphere.
      vec3 N = normalize(fragNormal);
      float w = (1.+dot(N,up))/2.;
      float reflectionCoefficient = 0.8;
      gl_FragColor = vec4(reflectionCoefficient*(w*skyColor+(1.-w)*groundColor),1);
    }
    `, 
  attributes: {
...
mat4.perspective
  ...
  main.variable(observer("computeViewMatrix")).define("computeViewMatrix", ["mat4","computeEye"], function(mat4,computeEye){return(
(angles, center, radius) => {
  //const center = objectDimensions.center;
  const up = [0, 1, 0];
  return mat4.lookAt([], computeEye(angles, center, radius), center,[0,1,0])
}
)});
...
(angles, center, radius) => { 
  ...
  return [center[0]+viewDirection[0], center[1]+viewDirection[1], center[2]+viewDirection[2]];
}
)});
